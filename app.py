...
# Function to get banner preview HTML
def get_banner_preview_html(banner_link, camp_name):
    """Get banner preview HTML"""
    if not banner_link or banner_link == 'N/A' or banner_link == '':
        return ""
    ...
    <div style="margin-top: 10px; margin-bottom: 15px;">
        ...
        ">
            ...
            <div style="margin-top: 5px; font-size: 10px; color: rgba(255,255,255,0.7); text-align: center;">
                ...
                <span onclick="copyToClipboard('{banner_link}')" 
        ...
        ">
            ...
            ">
                <div style="margin-bottom: 10px;">
                    ...
                    <button onclick="copyToClipboard('{banner_link}')"
                            style="
                                background: #2196F3;
                                color: white;
                                border: none;
...
def create_contest_card(row, camp_name_col, camp_type_col, start_date_col, end_date_col,
    ...
    # Get banner preview HTML if banner exists
    banner_preview_html = ""
    if banner_link and banner_link != 'N/A' and banner_link != '':
        banner_preview_html = get_banner_preview_html(banner_link, camp_name)
...
# Add JavaScript for copy functionality and modal handling
st.markdown("""
<script>
...
function copyToClipboard(text) {
    ...
    try {
        ...
        alert('⚠️ Could not copy link. Please copy manually.');
    } finally {
                    ...
                    if view_mode == "Cards View":
                        ...
                        for _, row in filtered_contests.iterrows():
                            card_html = create_contest_card(
                                row, camp_name_col, camp_type_col, start_date_col, end_date_col,
                                winner_date_col, kam_col, to_whom_col, eligibility_col,
                                gift_col, gift_link_col, banner_link_col, status=row['Status']
                            )
                            st.markdown(card_html, unsafe_allow_html=True)
                    else:
                        # Table view - ADDED GIFT, GIFT LINK, AND BANNER LINK COLUMNS
                        display_cols = []
                        if camp_name_col: display_cols.append(camp_name_col)
                        ...
                        if banner_link_col: display_cols.append(banner_link_col)  # ADDED
                        if start_date_col: display_cols.append(start_date_col)
                        if end_date_col: display_cols.append(end_date_col)
